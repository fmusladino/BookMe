...
/**
 * GET /api/admin/cupones — Lista todos los cupones.
 */
export async function GET() {
  ...
  try {
    ...
    if (error) {
      console.error("[GET admin/cupones]", error);
      return NextResponse.json({ error: "Error al obtener cupones" }, { status: 500 });
    }

    return NextResponse.json({ coupons: coupons || [], total: coupons?.length ?? 0 });
  } catch (error) {
    console.error("[GET admin/cupones]", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error("[POST admin/cupones]", error);
